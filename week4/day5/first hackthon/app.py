...
def render_game(vertical, horizontal, up_pipes, down_pipes, your_score):
    window.blit(
        pygame.transform.smoothscale(game_images["background"], window.get_size()),
        (0, 0),
    )
    for u_pipe, d_pipe in zip(up_pipes, down_pipes):
        ...
        sea_level_scaled = pygame.transform.scale(
            game_images["sea_level"],
            (window_width, game_images["sea_level"].get_height()),
        )
        window.blit(
            sea_level_scaled,
            (0, window_height - sea_level_scaled.get_height()),
        )
...
def show_welcome_screen():
    ...
    while True:
        ...
        window.blit(
            title_surf,
            (window_width / 2 - title_surf.get_width() / 2, window_height / 4),
        )
        window.blit(
            start_surf,
            (window_width / 2 - start_surf.get_width() / 2, window_height / 2),
        )
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
...
def main():
    ...
    while True:
        ...
        for u_pipe, d_pipe in zip(up_pipes, down_pipes):
            ...
            if (
                (
                    vertical < u_pipe["y"] + pipe_height
                    or vertical + game_images["flappydragon"].get_height() > d_pipe["y"]
                )
                and horizontal + game_images["flappydragon"].get_width() > u_pipe["x"]
                and horizontal < u_pipe["x"] + game_images["pipeimage"][0].get_width()
                and horizontal + game_images["flappydragon"].get_width() > d_pipe["x"]
                and horizontal < d_pipe["x"] + game_images["pipeimage"][0].get_width()
                or vertical + game_images["flappydragon"].get_height() > ground
            ):
                ...
                down_pipes = [
                    {"x": window_width + 200, "y": first_pipe[1]["y"]},
                    {
                        "x": window_width + 200 + (window_width / 2),
                        "y": second_pipe[1]["y"],
                    },
                ]
                up_pipes = [
